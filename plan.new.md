...
- [4. Ways of Working](#4-ways-of-working)
  ...
  - [4.2 The design handoff kit — what to send the supplier](#42-the-design-handoff-kit--what-to-send-the-supplier)
    ...
    - [4.2.7 What else we hand over](#427-what-else-we-hand-over)
  - [4.3 Conversion: design-src/ → Jinja](#43-conversion-design-src--jinja)
    - [4.3.1 The eight rewrites (a manual checklist, or the converter's spec)](#431-the-eight-rewrites-a-manual-checklist-or-the-converters-spec)
    - [4.3.2 After conversion, the Jinja file wins](#432-after-conversion-the-jinja-file-wins)
    - [4.3.3 Conversion acceptance checklist](#433-conversion-acceptance-checklist)
  - [4.4 Git and CI for a solo developer](#44-git-and-ci-for-a-solo-developer)
- [5. Roles & Permissions](#5-roles--permissions)
  - [5.1 Roles](#51-roles)
  - [5.2 Permission model](#52-permission-model)
  - [5.3 Governance rules (non-negotiable)](#53-governance-rules-non-negotiable)
- [6. User Journeys](#6-user-journeys)
  - [6.1 Journey A — Applicant (no account, applies, never returns unless selected)](#61-journey-a--applicant-no-account-applies-never-returns-unless-selected)
  - [6.2 Journey B — New student logging in for the first time](#62-journey-b--new-student-logging-in-for-the-first-time)
  - [6.3 Journey C — Administrator (daily, weekly, deadline, exam, post-result)](#63-journey-c--administrator-daily-weekly-deadline-exam-post-result)
  - [6.4 Journey D — Verifier (employer / university checking a certificate)](#64-journey-d--verifier-employer--university-checking-a-certificate)
- [7. Functional Requirements](#7-functional-requirements)
  - [7.1 Public site](#71-public-site)
  - [7.2 Application](#72-application)
  - [7.3 Admit card, result, certificate (public lookups)](#73-admit-card-result-certificate-public-lookups)
  - [7.4 Student portal](#74-student-portal)
  - [7.5 Admin console](#75-admin-console)
  - [7.6 Cross-cutting](#76-cross-cutting)
- [8. Screens](#8-screens)
  - [8.1 Public pages](#81-public-pages)
    - [8.1.1 Home /](#811-home-)
    - [8.1.2 /apply](#812-apply)
    - [8.1.3 /apply/success/<app_no>](#813-applysuccessapp_no)
    - [8.1.4 /admit-card, /results, /certificate-verification](#814-admit-card-results-certificate-verification)
    - [8.1.5 /verify/<code>](#815-verifycode)
    - [8.1.6 /live-status](#816-live-status)
  - [8.2 Student portal](#82-student-portal)
  - [8.3 Admin console](#83-admin-console)
- [9. Routes & URLs](#9-routes--urls)
  - [9.1 Sitemap](#91-sitemap)
...
- [10. Design System](#10-design-system)
  ...
  - [10.4 Typography](#104-typography)
  - [10.5 Signature elements (the deliberate, non-generic moves)](#105-signature-elements-the-deliberate-non-generic-moves)
  - [10.6 Explicit anti-slop rules for this project](#106-explicit-anti-slop-rules-for-this-project)
    - [10.6.1 Enforced by the Tailwind theme, not by discipline](#1061-enforced-by-the-tailwind-theme-not-by-discipline)
  - [10.7 Component inventory (built once, reused)](#107-component-inventory-built-once-reused)
...
- [12. Backend Architecture](#12-backend-architecture)
  ...
  - [12.2 App factory essentials](#122-app-factory-essentials)
  - [12.3 Database strategy](#123-database-strategy)
  - [12.4 Background work on shared hosting](#124-background-work-on-shared-hosting)
- [13. Data Model](#13-data-model)
  - [13.1 Identity & access](#131-identity--access)
    - [users](#users)
    - [login_attempts](#login_attempts)
    - [password_reset_tokens](#password_reset_tokens)
    - [user_sessions](#user_sessions)
    - [audit_logs](#audit_logs)
  - [13.2 Geography & reference data](#132-geography--reference-data)
    - [divisions](#divisions)
    - [districts](#districts)
    - [upazilas](#upazilas)
  ...
  - [13.3 Applicants & applications](#133-applicants--applications)
    - [applicants](#applicants)
    - [applications](#applications)
    - [13.3.1 Canonical status machine](#1331-canonical-status-machine)
  ...
  - [13.4 Exam & selection](#134-exam--selection)
    - [exams](#exams)
  ...
  - [13.5 Enrollment, training, certification](#135-enrollment-training-certification)
  ...
  - [13.6 Content & CMS](#136-content--cms)
  - [13.7 Live stats](#137-live-stats)
    - [stats_snapshots](#stats_snapshots)
    - [stats_targets](#stats_targets)
  - [13.8 Notifications](#138-notifications)
  - [13.9 System](#139-system)
  - [13.10 Schema totals & seed data](#1310-schema-totals--seed-data)
- [14. Service Layer](#14-service-layer)
- [15. Documents & PDF](#15-documents--pdf)
  - [15.1 Documents to produce](#151-documents-to-produce)
  - [15.2 Engine strategy — layered fallback (the safe answer)](#152-engine-strategy--layered-fallback-the-safe-answer)
  - [15.3 QR codes](#153-qr-codes)
  - [15.4 Signatures, seals and security printing](#154-signatures-seals-and-security-printing)
- [16. Authentication & Security](#16-authentication--security)
  - [16.1 The three identity types](#161-the-three-identity-types)
  - [16.2 Student login — exact mechanics](#162-student-login--exact-mechanics)
  - [16.3 Account provisioning (the selection gate)](#163-account-provisioning-the-selection-gate)
  - [16.4 Admin console hardening (vs the reference site's shared login)](#164-admin-console-hardening-vs-the-reference-sites-shared-login)
  - [16.5 Application-layer protections](#165-application-layer-protections)
  - [16.6 Security response headers (.htaccess + after_request)](#166-security-response-headers-htaccess--after_request)
  - [16.7 Privacy & legal compliance](#167-privacy--legal-compliance)
- [17. Notifications](#17-notifications)
  - [17.1 Channels](#171-channels)
  - [17.2 Bangla SMS reality check (important, affects budget)](#172-bangla-sms-reality-check-important-affects-budget)
  - [17.3 Message catalogue (all Bangla, all editable)](#173-message-catalogue-all-bangla-all-editable)
  - [17.4 Queue mechanics](#174-queue-mechanics)
- [18. Bangla Language & Fonts](#18-bangla-language--fonts)
  - [18.1 Locale architecture](#181-locale-architecture)
  - [18.2 Fonts (self-hosted — no CDN on a government site)](#182-fonts-self-hosted--no-cdn-on-a-government-site)
  - [18.3 Bangla typography rules enforced in CSS](#183-bangla-typography-rules-enforced-in-css)
  - [18.4 Input handling](#184-input-handling)
- [19. Performance, PWA & SEO](#19-performance-pwa--seo)
  - [19.1 PWA](#191-pwa)
  - [19.2 Performance budget](#192-performance-budget)
  - [19.3 SEO](#193-seo)
...
- [21. Deployment](#21-deployment)
  - [21.1 Hosting recommendation](#211-hosting-recommendation)
  - [21.2 One-time server setup (cPanel path)](#212-one-time-server-setup-cpanel-path)
  - [21.3 passenger_wsgi.py](#213-passenger_wsgipy)
  - [21.4 .htaccess (docroot — merged by hand, then NEVER overwritten by deploys)](#214-htaccess-docroot--merged-by-hand-then-never-overwritten-by-deploys)
  - [21.5 deploy.sh — complete specification](#215-deploysh--complete-specification)
  - [21.6 rollback.sh](#216-rollbacksh)
  - [21.7 GitHub Actions](#217-github-actions)
  - [21.8 Backup, monitoring and retention](#218-backup-monitoring-and-retention)
  - [21.9 Go-live cutover checklist](#219-go-live-cutover-checklist)
- [22. Operations Runbook](#22-operations-runbook)
  - [22.1 Who does what](#221-who-does-what)
  - [22.2 Daily checklist (10 minutes, on the admin dashboard)](#222-daily-checklist-10-minutes-on-the-admin-dashboard)
  - [22.3 The full admission cycle as an operator workflow](#223-the-full-admission-cycle-as-an-operator-workflow)
  - [22.4 Escalation & incident runbook](#224-escalation--incident-runbook)
  - [22.5 Handover documentation to produce](#225-handover-documentation-to-produce)
- [23. Reports & Analytics](#23-reports--analytics)
  - [23.1 Export handling](#231-export-handling)
  - [23.2 Dashboard KPIs (admin home)](#232-dashboard-kpis-admin-home)
  - [23.3 Public live-status metrics (Sylhet scope)](#233-public-live-status-metrics-sylhet-scope)
  - [23.4 HQ / national sync export](#234-hq--national-sync-export)
- [24. Testing & QA](#24-testing--qa)
  - [24.1 Test pyramid](#241-test-pyramid)
  - [24.2 The E2E journeys that must pass](#242-the-e2e-journeys-that-must-pass)
  - [24.3 Regression test fixtures worth gold](#243-regression-test-fixtures-worth-gold)
  - [24.4 Anti-slop and quality gates for the UI](#244-anti-slop-and-quality-gates-for-the-ui)
- [25. Roadmap](#25-roadmap)
  - [25.1 The honest capacity finding — read this before committing to the plan](#251-the-honest-capacity-finding--read-this-before-committing-to-the-plan)
  - [25.2 Scope levers — what to cut if 22 weeks is too long](#252-scope-levers--what-to-cut-if-22-weeks-is-too-long)
  - [25.3 Staged launch — which matters more than the total](#253-staged-launch--which-matters-more-than-the-total)
- [26. Risks](#26-risks)
- [27. Open Questions](#27-open-questions)
  - [27.1 Provisional defaults — what we build against until you answer](#271-provisional-defaults--what-we-build-against-until-you-answer)
- [28. Appendices](#28-appendices)
  - [28.1 Appendix A — Sylhet Division geography seed data](#281-appendix-a--sylhet-division-geography-seed-data)
  - [28.2 Appendix B — Roll / Registration / Certificate number formats](#282-appendix-b--roll--registration--certificate-number-formats)
  - [28.3 Appendix C — Forms of the two login paths (for the implementer)](#283-appendix-c--forms-of-the-two-login-paths-for-the-implementer)
  - [28.4 Appendix D — Permission → route matrix (abridged)](#284-appendix-d--permission--route-matrix-abridged)
  - [28.5 Appendix E — Definition of Done (per feature)](#285-appendix-e--definition-of-done-per-feature)
  - [28.6 Appendix F — Recommended first three commits](#286-appendix-f--recommended-first-three-commits)
  - [28.7 Appendix G — What I need from you to start coding](#287-appendix-g--what-i-need-from-you-to-start-coding)
...
| PDF | **HTML print views (guaranteed)** + WeasyPrint/Playwright if available | See §15 — biggest technical risk |
...
| **CSS build** | Tailwind standalone CLI, output **committed to the repo** | Zero Node/Docker needed on cPanel — see §11.1 |
...
**~10–13 weeks** for one competent developer; ~7–8 weeks with two in parallel (§25).
...
1. **Student and admin share one login form and are separated only by role after auth.**
   This is a **weakness we will not copy** (§16) — staff and candidates should not be
...
2. **Districts are not fetched by AJAX.** The full division→district JSON map is *embedded in
   ...
   (§9), which is faster and keeps one source of truth.
...
| Roles | admin/student | **7 granular roles** (§5) |
...
- Merging into the national database (a sync **export** is provided, §23.4).
...
### 4.1 Development model: solo build with an external design supplier
...
| `assets/tailwind/source.css` | **You** | The design tokens **and** the component classes. The supplier must not edit it — they request changes (§4.2.5). |
| `app/static/css/app.css` | **You** | Compiled artifact, committed. |
| `app/templates/public/**` | **You** | Born from `design-src/public/**` via conversion (§4.3). |
...
|---|---|---|
| **C1** | **URL map** — every public route and its final path | §9.2 (public rows) + `design-src/content/routes.md` |
| **C2** | **Design tokens** — the complete colour, type, radius, shadow and spacing scale | `assets/tailwind/source.css` (§10.3) |
...
DO NOT use arbitrary values: bg-[#123456], w-[373px], text-[19px], r-[24px].
   These bypass the design system and break the anti-slop guarantees in §10.6.1.
...
DO NOT use rounded-3xl, shadow-lg, backdrop-blur, gradients, or emoji as icons.
   Those utilities do not exist in this theme — see §10.6.1.
...
### 4.3 Conversion: `design-src/` → Jinja
...
#### 4.3.1 The eight rewrites (a manual checklist, or the converter's spec)
...
| 6 | Repeated components | Recognised class-clusters (button, card, badge, table) → the matching Jinja macro call from §11.1.5 |
...
#### 4.3.2 After conversion, the Jinja file wins
...
#### 4.3.3 Conversion acceptance checklist

```
□ Zero unresolved [[TOKENS]] (converter exits 0)
□ Zero lorem/English filler in user-visible text
□ Every href resolves to a route that exists in §9.2
...
### 4.4 Git and CI for a solo developer
...
**Pre-deploy validation (recommended, and a change from §21.7).** We keep `push to main` as the
...
### 5.3 Governance rules (non-negotiable)
...
### 6.1 Journey A — Applicant (no account, applies, never returns unless selected)
...
### 6.2 Journey B — New student logging in for the first time
...
### 6.3 Journey C — Administrator (daily, weekly, deadline, exam, post-result)

Covered in full in §22 (runbook). Summary:
...
### 6.4 Journey D — Verifier (employer / university checking a certificate)
...
### 7.3 Admit card, result, certificate (public lookups)
...
#### 8.1.4 `/admit-card`, `/results`, `/certificate-verification`
...
| 84 | GET | `/<PREFIX>/reports/<name>` | ✔ | `report.read` | §23 |
...
### 9.3 URL naming and Bangla labels policy
...
**structurally impossible to write**. The anti-slop rules in §10.6 stop being a code-review
...
### 10.3.1 Utility mapping cheat-sheet
...
**Tailwind implementation of the scale:** the `--text-*` overrides in §10.3 mean `text-base`
...
### 10.5 Signature elements (the deliberate, non-generic moves)
...
### 10.6 Explicit anti-slop rules for this project
...
#### 10.6.1 Enforced by the Tailwind theme, not by discipline
...
committed `app.css` is **byte-identical to a fresh build** (drift detector, see §11.1).

### 10.7 Component inventory (built once, reused)
...
### 10.8 Accessibility targets (WCAG 2.1 AA)
...
### 11.1 Tailwind architecture & build pipeline
...
#### 11.1.2 Why the standalone CLI + committed output
...
#### 11.1.3 The three build commands (in `package.json`)
...
#### 11.1.5 Jinja component macros (not a Python UI library)
...
Every component in §10.7 gets this treatment. **Consequence:** a variant that violates the design
system (e.g. a purple button) cannot be added without inventing a colour that does not exist.

#### 11.1.6 Accessibility mechanics in Tailwind
...
- Touch targets: the `sizes` map in §11.1.5 floors at `h-9` (36px) for dense admin tables and
...
> `git diff --exit-code app/static/css/app.css` catches it if you forget anyway (§21.7).
...
### 12.1 Directory layout (mirrors your proven Favonia pattern)
...
├── design-src/                     ← DESIGN SUPPLIER'S DELIVERY AREA (see §4.1–§4.3)
...
│   ├── kitchen-sink.html           component gallery: every component, every state (§4.2)
...
### 12.4 Background work on shared hosting
...
#### 13.3.1 Canonical status machine
...
### 13.5 Enrollment, training, certification
...
| `pdf_service` | Engine abstraction + Bangla text safety | `render(html, base_url, out_path)` with engine detection (§15) |
...
### 15.2 Engine strategy — layered fallback (the safe answer)
...
### 15.4 Signatures, seals and security printing
...
### 16.2 Student login — exact mechanics
...
### 16.3 Account provisioning (the selection gate)
...
### 16.4 Admin console hardening (vs the reference site's shared login)
...
### 16.5 Application-layer protections
...
### 16.6 Security response headers (`.htaccess` + `after_request`)
...
### 17.2 Bangla SMS reality check (important, affects budget)
...
### 17.3 Message catalogue (all Bangla, all editable)
...
## 18. Bangla Language & Fonts

### 18.1 Locale architecture
...
### 18.2 Fonts (self-hosted — no CDN on a government site)
...
### 18.3 Bangla typography rules enforced in CSS
...
**Tailwind-specific budget guard:** the wiped default palette (§10.3) removes roughly a third of
...
### 21.2 One-time server setup (cPanel path)
...
### 21.4 `.htaccess` (docroot — merged by hand, then NEVER overwritten by deploys)
...
### 21.5 `deploy.sh` — complete specification
...
SYNC (excluding the list above, plus __pycache__, *.pyc, .venv, node_modules, .github,
  ...
  #       files incapable of affecting production. See §4.1.1.
...
### 21.8 Backup, monitoring and retention
...
### 21.9 Go-live cutover checklist
...
### 22.2 Daily checklist (10 minutes, on the admin dashboard)
...
### 22.3 The full admission cycle as an operator workflow
...
### 22.4 Escalation & incident runbook
...
### 22.5 Handover documentation to produce
...
### 23.2 Dashboard KPIs (admin home)
...
### 23.3 Public live-status metrics (Sylhet scope)
...
| Integration | `pytest` + Flask test client | every route: status codes, permissions, redirects, validation | every route in §9.2 has ≥ 1 test |
...
| Security | `bandit`, `pip-audit`, manual checklist | §16 controls | zero high findings |
| Manual UAT | Bangla checklist | 18-step operator workflow with a real batch of 50 | signed off |

### 24.2 The E2E journeys that must pass
...
### 24.3 Regression test fixtures worth gold
...
### 24.4 Anti-slop and quality gates for the UI

Before any screen is called done:
- No item from the §10.6 banned list is present — and CI's `tools/check-css.py` proves the
  compiled stylesheet contains no banned construct (§10.6.1), so this is enforced, not hoped for.
...
You are the **sole developer** (§4.1). The public-site markup arrives from the design supplier, so
those days are theirs, not yours. Everything below is **your** effort. Days are working days, not
elapsed days.

| # | Milestone | Deliverable | Your days |
|---|---|---|---|
| **M0** | Discovery & **contract freeze** | Plan signed off; **C1–C4 frozen** (§4.1.3); server, domain and DB provisioned | 3 |
...
### 25.1 The honest capacity finding — read this before committing to the plan
...
risk in §26 has a mitigation. There is no mitigation for "one person, 108 days". So the levers are
...
### 25.2 Scope levers — what to cut if 22 weeks is too long
...
### 25.3 Staged launch — which matters more than the total
...
|---|---|---|---|---|
| **R1** | **Bangla text renders broken in server-side PDFs** (libpango unavailable on shared hosting) | **High** | High | Layered engine strategy + **guaranteed HTML print views** for every document (§15). Verify `PDF_ENGINE` capability **on day one of M2**, not at the end. |
...
| **R19** | **Conversion rework** — the supplier's standalone HTML does not convert cleanly, so screens bounce back | **High** | Medium | The handoff kit (§4.2) plus the `[[TOKEN]]` convention and the eight rewrite rules used as a manual checklist (§4.3.1). **Build `tools/convert-design.py` once any §4.3 threshold is crossed** (>25 screens, same screen re-delivered twice, or a second programme reuses the codebase). |
| **R20** | **Solo capacity** — 108 developer-days on one person is ~22 weeks best case, 24–27 realistic | **High** | **High** | Quantified in §25.1 and therefore chosen deliberately rather than discovered. Levers are **scope, not people** (§25.2). Staged launch (§25.3) protects the deadline-critical Stage 1. |
...
| **R22** | **Supplier pushes to `main`** and it auto-deploys | Low | High | Structurally mitigated: `design-src/**` is excluded from the deploy sync (§21.5), so their files cannot reach the server at all. Your own `app/**` changes are gated by the blocking `flask render-check --all` pre-deploy job (§21.7). |
| **R23** | **Design drift** — the supplier invents components, colours or variants not in the system | Medium | Medium | The palette/radius/shadow namespaces are wiped, so invented colours **do not compile**; the "do not" list (§4.2.6); `kitchen-sink.html` as the shared vocabulary; `DESIGN-NOTES.md` as the sanctioned request channel. |
...
### 27.1 Provisional defaults — what we build against until you answer
...
| **Q12** District coordinators | **Yes — separate logins with district-only row scoping** | Already fully designed in §5.2; `users.district_id` exists | ~0 (already built) |
...
1. **Q13 — no placeholder photography.** A grey "image goes here" box makes a design look broken and
   ...
   band is a complete composition that your logo can later *improve*, not complete. This is why §10.6
...
### 28.1 Appendix A — Sylhet Division geography seed data
...
### 28.2 Appendix B — Roll / Registration / Certificate number formats
...
### 28.3 Appendix C — Forms of the two login paths (for the implementer)
...
### 28.4 Appendix D — Permission → route matrix (abridged)
...
### 28.5 Appendix E — Definition of Done (per feature)
...
### 28.6 Appendix F — Recommended first three commits
...
### 28.7 Appendix G — What I need from you to start coding

| # | Item |
|---|---|
| 1 | Answers to the blocking questions in §27 (at minimum **Q1, Q2, Q7, Q13**) |
